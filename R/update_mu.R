update_mu <- function(tree, y, z, strata, sc1 = numeric(length(y)),
                      lambda = numeric(length(y)), sigma2_mu){

  # Log-likelihood and gradient functions needed for ars
  ll <- function(m, y, z, strata, sc1, lambda, sigma2_mu, max_win, na_locs){
    comp_loglik(y, sc = sc1 + z * (m + lambda), strata, max_win, na_locs) - ((m^2) / (2*sigma2_mu))
  }
  grd <- function(m, y, z, strata, sc1, lambda, sigma2_mu, max_win, na_locs){
    comp_grd(y, z, sc1 + z * (m + lambda), strata, max_win, na_locs) - (m / sigma2_mu)
  }
  ...
  for(n in leaves){
    ...
    tree$nodes[[n]]$mu <- ars::ars(n = 1, f = ll_V, fprima = grd_V,
                                   y = y[id], z = z[id], strata = strata[id],
